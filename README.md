...
func main() {
	...
	// Confined I/O through an *os.Root: name is relative to the root,
...
- `ReadBoundedInRoot(ctx, root, name, maxBytes) ([]byte, error)`: the confined read, and the read-side counterpart to `WriteFileInRoot`: a symlink or `..` component in `name` cannot redirect the read out of the root's tree. Only a regular file is read, and the open is non-blocking, so a FIFO planted in the tree is refused with `ErrNotRegular` instead of stalling the caller. The caller owns `root`.
...
- `WalkDirInRoot(ctx, root, fn fs.WalkDirFunc) error`: stream a confined tree. Entries arrive in fixed batches rather than as one materialized inventory per directory, so a large or hostile directory is never resident before the callback can refuse it; a named pipe planted where a directory was is refused instead of blocking the walk; a symlinked directory is never descended. The callback is an `fs.WalkDirFunc`, called exactly as `fs.WalkDir` calls one: the root first as `"."`, then each entry pre-order, a directory that cannot be opened or finished reported through the callback for its own path, `fs.SkipDir`/`fs.SkipAll` honoured. Two deliberate differences: entries arrive in **directory order**, sorted within a batch only, and a directory's own entries are all visited before the walk descends into its subdirectories. The walk stops between batches once `ctx` is done; a caller wanting per-entry cancellation checks `ctx` in the callback. Walk a subtree by passing `root.OpenRoot(sub)`.
...
- `CleanupStaleTempsInRoot(ctx, root, maxAge, opts ...Option) (SweepResult, error)`: the confined sweep. It uses both of the above: enumerating through `WalkDirInRoot` (so a large or hostile directory is not materialized before the sweep sees its first entry) and unlinking every candidate through a pinned parent, so an ancestor swapped for a symlink cannot redirect a removal at another file inside the tree. Same signature shape, outcome model and `maxAge` rule as `CleanupStaleTemps`; the one difference is that confinement.
...
- **`mkdir` 0700, remembering whether _this_ call created the level.** Every decision below turns on that distinction, which `os.MkdirAll` cannot report.
...
> and re-opens the window the check closed. To close that gap, hold the directory
> open for its whole lifetime and act through an `*os.Root`, naming only the
> leaves inside it.
...
- `CleanupStaleTemps(ctx, dir, maxAge, opts ...Option) (SweepResult, error)`: remove orphaned temp files left by interrupted writes, reporting `Removed` / `Failed` / `Unreadable`. Only files matching the package's exact temp shape (`.atomicfile-<digits>.tmp`) and older than `maxAge` are reclaimed; caller-owned files that merely share the prefix or suffix (`.atomicfile-notes.tmp`, `config.tmp-backup`) are never touched. Each candidate is re-checked immediately before removal, so a same-named fresh temp created during the scan is spared. The sweep stops once `ctx` is done. Same signature shape and outcome model as `CleanupStaleTempsInRoot`; the one difference is confinement. See [Orphaned temps](#orphaned-temps) for what leaves one behind and how to pick `maxAge`.
...
They also refuse a target already occupied by anything else that is not a **regular file**, returning `ErrNotRegular`: the same verdict `ReadBoundedInRoot` gives what it will read and `RemoveFileInRoot` gives what it will unlink. `rename(2)` overwrites any non-directory, so without the refusal a write could replace a FIFO, a socket or a device node this package never created. An existing regular file is still overwritten, which is the point of the package. The check reads the target before it renames, because `rename(2)` has no "only if the destination is a regular file" mode, so its value is refusing the states that are there to be found, not winning a race.

Reads behave differently: `ReadBounded` follows symlinks by design (`os.Open` resolves them), so it does NOT refuse a symlink at `path`. When reading from a directory writable by a less-trusted principal, confine the path yourself: open the file through an `*os.Root` and read it with `ReadBoundedFile`, which applies the same size and context bounds without following symlinks out of the root. `OpenRegularInRoot` is that open, already written; `OpenRegular` is the ambient-path form for a caller holding a full path into a directory it trusts, and it is the one read entry point that does refuse a symlink under the name: `O_NOFOLLOW` has the kernel refuse the open, which a check-then-open sequence cannot do without a race.

An `*os.Root` does not _pin_ a multi-component name: `OpenParentInRoot` and `RemoveFileInRoot` do, see [Confined Traversal and Removal](#confined-traversal-and-removal).
